...
class StockTransportRate(models.Model):
    ...
    def _check_overlap(self):
        for transport in self:
            overlapped_rate = self.search([('partner_id','=',transport.partner_id.id),('start_date','<=',transport.start_date)
                ,('end_date','>=',transport.end_date),('src_location_id','=',transport.src_location_id.id)
                ,('dest_location_id','=',transport.dest_location_id.id),('rate_type','=',transport.rate_type),('id','!=',transport.id)])
            if overlapped_rate:
